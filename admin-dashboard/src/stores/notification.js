...
export const useNotificationStore = defineStore('notification', () => {
  ...
  function buildMsg(event, data) {
    ...
    const map = {
      'new_transaction':       `New ${data.document_type || 'Document'} request submitted by ${name}`,
      'new_equipment_request': `New Equipment request submitted by ${name}`,
      'new_feedback':          data.rating ? `New feedback received — rated ${data.rating}/5 stars` : `New feedback received from ${name}`,
      'new_id_application':    `New ID Application submitted by ${name}`,
      'new_lost_card_report':  `Lost card reported by ${name}`,
      'new_rfid_linked':       `New RFID card linked for ${name}`,
    }
