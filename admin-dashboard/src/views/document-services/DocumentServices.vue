...
// ======================================
// ID Application — Ensure row exists
// ======================================
...
// ======================================
// ID Application — Template Upload
// ======================================
...
// Replace via plain file input (shown after a template exists)
...
// ======================================
// ID Application — Requirements modal
// Uses the same updateDocumentRequirements (PUT /documents/types/{id})
// as regular document types — no separate endpoint needed.
// ======================================
...
// ======================================
// CRUD Handlers (regular services)
// ======================================
...
// ======================================
// Template Upload/Download (regular)
// ======================================
...
// ======================================
// Field / Requirements Modals (regular)
// ======================================
  ...
  return h("div", { class: "relative group/tip inline-flex" }, [
    ...
    h("div", {
      class:
        ...
        "transition-all duration-200 bg-[#013C6D] text-[#E5F5FF] text-xs px-2 py-1 rounded " +
...
const columns = computed(() => [
  ...
  },
  {
    ...
    render(row) {
      if (editingId.value === row.id) {
        return h(NInput, { value: row.request_type_name, onUpdateValue(v) { row.request_type_name = v; } });
      }
  ...
  },
  {
    ...
    render(row) {
      if (editingId.value === row.id) {
        return h(NInput, { value: row.price, type: "number", onUpdateValue(v) { row.price = Number(v); } });
      }
    ...
    render(row) {
      return h("button", {
        ...
        class: `px-3 py-1 rounded-full text-xs font-medium cursor-pointer transition ${
          row.available ? "bg-green-100 text-green-800 hover:bg-green-200" : "bg-gray-100 text-gray-800 hover:bg-gray-200"
        }`,
...
<template>
  <div class="flex flex-col p-6 bg-white rounded-md w-full h-full overflow-hidden animate-fade-in">
    ...
    <template v-else>
      ...
      <!-- ============================================ -->
      <!-- ID APPLICATION — Pinned Special Section      -->
      <!-- ============================================ -->
      <div class="mb-4 rounded-xl border border-indigo-200 bg-indigo-50/60 overflow-hidden">
        ...
        >
          ...
          <div class="flex items-center gap-3">
            ...
            <span class="text-xs text-indigo-400 font-medium">
              Fee: ₱{{ parseFloat(idLocalPrice).toFixed(2) }}
            </span>
            <ChevronDownIcon
              class="w-4 h-4 text-indigo-400 transition-transform duration-200"
              :class="idPanelOpen ? 'rotate-180' : ''"
            />
          </div>
        </button>
        ...
        <div v-if="idPanelOpen" class="border-t border-indigo-200 bg-white">
          <div class="p-5 flex gap-6 items-start">

            <!-- Template area -->
            <div class="flex-1 min-w-0">

              <!-- No template: drag-and-drop uploader -->
              <template v-if="!idHasTemplate">
                <NUpload
                  :custom-request="handleIDFileUpload"
                    ...
                    <div class="flex flex-col items-center py-8 gap-2">
                      ...
                      <template v-else>
                        ...
                        <p class="text-sm font-medium text-gray-600">
                          Click or drag a <span class="text-indigo-600">.docx</span> here
                        </p>
                ...
                <div class="flex items-center gap-4 p-4 bg-gray-50 border border-gray-200 rounded-lg">
                  ...
                  <div class="flex-1 min-w-0">
                    ...
                    <div class="flex items-center gap-2 mt-3">
                      ...
                      >
                        <NSpin v-if="idUploading" :size="12" />
            ...
            <!-- Modals -->
            <div class="flex flex-col gap-3 w-52 flex-shrink-0">
              <button
                @click="showIdFieldModal = true"
                class="flex items-center gap-2 text-sm text-blue-600 border border-blue-200 hover:bg-blue-50 px-3 py-2 rounded-lg transition w-full text-left"
              >
                ...
                <span class="text-xs bg-blue-100 text-blue-600 px-1.5 py-0.5 rounded-full font-medium">
                  {{ idDocType?.fields?.length ?? 0 }}
                </span>
              </button>
              <button
                @click="showIdReqModal = true"
                class="flex items-center gap-2 text-sm text-purple-600 border border-purple-200 hover:bg-purple-50 px-3 py-2 rounded-lg transition w-full text-left"
              >
                <ClipboardDocumentListIcon class="w-4 h-4 flex-shrink-0" />
                <span class="flex-1">Requirements</span>
                <span class="text-xs bg-purple-100 text-purple-600 px-1.5 py-0.5 rounded-full font-medium">
                  {{ idDocType?.requirements?.length ?? 0 }}
                </span>
              </button>
            </div>

            <!-- Price -->
            <div class="flex flex-col gap-3 w-36 flex-shrink-0">
              <!-- Fee -->
              <label class="text-xs font-medium text-gray-500 mb-1 block">Application Fee (₱)</label>
              <div class="flex flex-col gap-2">
                <NInput
                  v-model:value="idLocalPrice"
                  type="number"
                  size="small"
                  :min="0"
                  :step="0.01"
                  placeholder="0.00"
                />
                <button class="px-3 py-1 border border-blue-600 text-blue-600 rounded-md font-medium text-xs hover:bg-blue-50 transition items-center text-center" :loading="idSaving" @click="idSavePrice">
                  Save
                </button>
              </div>
            </div>
  ...
  <!-- ID Template Preview Modal -->
  <NModal
    :show="showIdPreviewModal"
    @update:show="showIdPreviewModal = false"
    :mask-closable="true"
  >
    <div class="bg-white rounded-xl shadow-xl flex flex-col overflow-hidden" style="width: 860px; height: 90vh;">
      ...
      <div class="flex items-center justify-between px-5 py-3 border-b bg-gray-50 flex-shrink-0">
        <div class="flex items-center gap-2">
          ...
          <span class="text-xs text-gray-400">· id-application-template.docx</span>
        </div>
        <div class="flex items-center gap-2">
          <button
            @click="handleIDDownloadTemplate"
          ...
          <button
            @click="showIdPreviewModal = false"
