...
// ======================================
const colorMap = {
  blue:   { header: 'bg-blue-50 border-b border-blue-100',     icon: 'text-blue-600',   primary: 'bg-blue-100 text-blue-700 border border-blue-200',     alias: 'bg-blue-50 text-blue-600 border border-blue-100' },
  violet: { header: 'bg-violet-50 border-b border-violet-100', icon: 'text-violet-600', primary: 'bg-violet-100 text-violet-700 border border-violet-200', alias: 'bg-violet-50 text-violet-600 border border-violet-100' },
  green:  { header: 'bg-green-50 border-b border-green-100',   icon: 'text-green-600',  primary: 'bg-green-100 text-green-700 border border-green-200',   alias: 'bg-green-50 text-green-600 border border-green-100' },
  orange: { header: 'bg-orange-50 border-b border-orange-100', icon: 'text-orange-600', primary: 'bg-orange-100 text-orange-700 border border-orange-200', alias: 'bg-orange-50 text-orange-600 border border-orange-100' },
  teal:   { header: 'bg-teal-50 border-b border-teal-100',     icon: 'text-teal-600',   primary: 'bg-teal-100 text-teal-700 border border-teal-200',     alias: 'bg-teal-50 text-teal-600 border border-teal-100' },
  red:    { header: 'bg-red-50 border-b border-red-100',       icon: 'text-red-600',    primary: 'bg-red-100 text-red-700 border border-red-200',     alias: 'bg-red-50 text-red-600 border border-red-100' },
}
</script>
...
<template>
  <div class="flex flex-col p-6 bg-white rounded-md w-full h-full overflow-hidden">
    ...
    <div class="flex-shrink-0 mb-5">
      ...
      <p class="text-sm text-gray-500 mt-1">System reference guide for the Barangay Kiosk Admin Dashboard.</p>
    </div>
    ...
    <div class="flex-1 overflow-y-auto">
      <div class="max-w-4xl mx-auto space-y-4 animate-fade-in" :key="activeSection">
        ...
        <template v-if="activeSection === 'about'">
          ...
          <!-- Hero banner -->
          <div class="relative bg-blue-600 rounded-xl p-6 text-white overflow-hidden">
            <div class="absolute inset-0 opacity-10 pointer-events-none">
              <svg viewBox="0 0 400 200" class="w-full h-full" preserveAspectRatio="xMaxYMid slice">
                <circle cx="380" cy="10" r="130" stroke="white" stroke-width="1.5" fill="none"/>
                <circle cx="380" cy="10" r="85"  stroke="white" stroke-width="1.5" fill="none"/>
                <circle cx="380" cy="10" r="40"  fill="white"/>
              </svg>
            </div>
            <div class="relative">
              <div class="flex flex-wrap gap-2 mb-3">
                <span class="bg-white/20 text-white text-xs font-bold px-2.5 py-1 rounded-full">PUP Thesis Project</span>
                <span class="bg-white/20 text-white text-xs font-bold px-2.5 py-1 rounded-full">November 2025</span>
              </div>
              <h2 class="text-xl font-bold leading-snug mb-2">
                RFID-Enabled Barangay Transaction Kiosk<br/>with SMS Notification System
              </h2>
              <p class="text-blue-100 text-sm leading-relaxed max-w-xl">
                A self-service platform for Poblacion I, Amadeo, Cavite — designed to modernize barangay transactions through RFID authentication, automated form handling, and GSM-based SMS notifications.
              </p>
              <div class="mt-4 flex flex-wrap gap-2">
                <span
                  v-for="tag in ['Vue.js', 'Node.js', 'Raspberry Pi 5', 'RFID Reader', 'GSM Module', 'Offline-First', 'Local LAN']"
                  :key="tag"
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="bg-white rounded-xl border border-gray-200 p-5">
              <h3 class="font-bold text-gray-800 text-sm mb-3 flex items-center gap-2">
                <InformationCircleIcon class="w-4 h-4 text-blue-600" /> Project Background
              </h3>
              <p class="text-sm text-gray-600 leading-relaxed">
                Barangay service delivery has long relied on manual processes — paper forms, manual encoding, and verbal announcements. This system addresses those inefficiencies with a centralized, offline-capable digital platform covering document requests, equipment borrowing, resident management, and community communication.
              </p>
            </div>
            <div class="bg-white rounded-xl border border-gray-200 p-5">
              <h3 class="font-bold text-gray-800 text-sm mb-3 flex items-center gap-2">
              ...
              <ul class="text-sm text-gray-600 space-y-1.5">
                ...
                ]" :key="f" class="flex items-start gap-2">
                  <span class="text-blue-600 font-bold mt-0.5 leading-none">•</span>{{ f }}
                </li>
              </ul>
            </div>
          </div>
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
              <div
                v-for="c in [
                  ...
                  { name: 'Admin Dashboard',  sub: 'This interface · LAN-connected' },
                ]" :key="c.name"
                class="rounded-lg border border-blue-100 bg-blue-50 p-3 text-center"
              >
                <div class="font-bold text-xs text-blue-700 mb-1">{{ c.name }}</div>
                <div class="text-xs text-blue-500">{{ c.sub }}</div>
              </div>
            </div>
            <p class="text-xs text-gray-500 mt-3 leading-relaxed">
              The kiosk communicates with this dashboard over a local wireless LAN. No internet connection is required for any core operation. The GSM module handles outbound SMS independently through the cellular network.
            </p>
          </div>
          ...
          <div class="bg-white rounded-xl border border-gray-200 p-5">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
              <div
                v-for="name in ['Alleah Marie G. Bayas', 'Justine Mae L. Convicto', 'Eloissa S. Francisco', 'Keanno Brennel A. Macatangay']"
                :key="name"
                class="text-center bg-gray-50 rounded-lg p-3 border border-gray-100"
              >
                <div class="w-10 h-10 rounded-full bg-blue-600 text-white flex items-center justify-center text-sm font-bold mx-auto mb-2">
                  {{ name.charAt(0) }}{{ name.split(' ').at(-1).charAt(0) }}
                </div>
                <div class="text-xs font-semibold text-gray-700 leading-tight">{{ name }}</div>
              </div>
            </div>
            <p class="text-xs text-gray-400 text-center mt-3">
              BS Computer Engineering · Polytechnic University of the Philippines · 2025
            </p>
          </div>
        </template>
        ...
        <template v-if="activeSection === 'guide'">
          ...
          <div class="flex gap-3 bg-amber-50 border border-amber-200 rounded-xl p-4">
            ...
            <div>
              <p class="text-sm font-semibold text-amber-800">Admin Dashboard Documentation Only</p>
              <p class="text-xs text-amber-700 mt-0.5 leading-relaxed">
                This guide covers this admin dashboard only. For hardware setup, kiosk physical installation, and network configuration, refer to the separate <strong>Hardware & Deployment Manual</strong> provided with the project deliverables.
              </p>
            </div>
          </div>

          <div v-for="mod in guideModules" :key="mod.id"
            ...
            >
              <div class="flex items-center gap-3">
                ...
                <div>
                  <div class="font-semibold text-sm text-gray-800">{{ mod.title }}</div>
                  <div class="text-xs text-gray-400 mt-0.5">{{ mod.summary }}</div>
                </div>
              </div>
              <ChevronDownIcon
            ...
            <div v-if="openGuideItems.has(mod.id)" class="px-5 pb-5">
              <div class="border-t border-gray-100 pt-4 text-sm text-gray-600 leading-relaxed whitespace-pre-line">{{ mod.content }}</div>
            </div>
          </div>
          ...
          <div class="bg-white rounded-xl border border-gray-200 p-5">
            <h3 class="font-semibold text-gray-800 text-sm mb-3 flex items-center gap-2">
              <LightBulbIcon class="w-4 h-4 text-blue-600" /> Quick Tips & Best Practices
            </h3>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              >
                <div class="font-semibold text-xs text-blue-600 mb-1">{{ tip.title }}</div>
                <div class="text-xs text-gray-500 leading-relaxed">{{ tip.text }}</div>
              </div>
            </div>
          </div>
        </template>
        ...
        <template v-if="activeSection === 'placeholders'">
          ...
          <div class="bg-blue-600 rounded-xl p-5 text-white">
            <div class="flex items-start gap-3">
              ...
              <div>
                <h3 class="font-bold text-sm mb-1">Document Template Placeholder Reference</h3>
                <p class="text-blue-100 text-xs leading-relaxed">
                  When uploading a <strong>.docx document template</strong>, insert placeholders using double curly braces —
                  e.g. <code class="bg-white/20 px-1.5 py-0.5 rounded font-mono">&#123;&#123;full_name&#125;&#125;</code>.
                  The system replaces them with the resident's actual profile data when a request is submitted.
                </p>
                <p class="text-blue-200 text-xs mt-2">
                  Any <strong>alias</strong> listed for a field works identically. Click any tag below to copy it as a ready-to-paste placeholder.
                </p>
              </div>
            </div>
          </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
            ...
            >
              <div class="font-semibold text-xs text-gray-700 mb-1.5 flex items-center gap-2">
                <span class="w-5 h-5 bg-blue-600 text-white text-xs rounded-full flex items-center justify-center font-bold flex-shrink-0">{{ i + 1 }}</span>
                {{ step.title }}
              </div>
          ...
          <!-- Placeholder groups -->
          <div v-for="group in placeholderGroups" :key="group.group"
            class="bg-white rounded-xl border border-gray-200 overflow-hidden">

            <div class="px-5 py-3 flex items-center gap-2" :class="colorMap[group.color].header">
              <component :is="group.icon" class="w-4 h-4" :class="colorMap[group.color].icon" />
              <span class="font-semibold text-sm text-gray-800">{{ group.group }}</span>
              <span class="ml-auto text-xs text-gray-400">{{ group.placeholders.length }} field{{ group.placeholders.length !== 1 ? 's' : '' }}</span>
            </div>

            <div class="divide-y divide-gray-50">
              <div v-for="ph in group.placeholders" :key="ph.key"
                  ...
                  <button
                    ...
                    :class="[
                      ...
                      copiedKey === ph.key
                        ...
                        : colorMap[group.color].primary + ' hover:opacity-80 cursor-pointer'
                    ]"
                ...
                <p class="text-xs text-gray-500 leading-relaxed">{{ ph.description }}</p>
              </div>
            </div>
          </div>
          ...
          <div class="flex gap-3 bg-amber-50 border border-amber-200 rounded-xl p-4">
            <ExclamationTriangleIcon class="w-5 h-5 text-amber-500 flex-shrink-0 mt-0.5" />
            <div>
              <p class="text-sm font-semibold text-amber-800 mb-1.5">Important Notes</p>
              <ul class="text-xs text-amber-700 space-y-1.5 leading-relaxed">
                <li>• <strong>Exact spelling required.</strong> Placeholders are case-sensitive. <code class="bg-amber-100 px-1 rounded font-mono">Full_Name</code> will NOT work — use <code class="bg-amber-100 px-1 rounded font-mono">full_name</code>.</li>
                <li>• <strong>Click any tag to copy it</strong> as a ready-to-paste <code class="bg-amber-100 px-1 rounded font-mono">&#123;&#123;placeholder&#125;&#125;</code> string.</li>
                <li>• <strong>Aliases are interchangeable.</strong> <code class="bg-amber-100 px-1 rounded font-mono">&#123;&#123;name&#125;&#125;</code> and <code class="bg-amber-100 px-1 rounded font-mono">&#123;&#123;full_name&#125;&#125;</code> both produce the resident's full name.</li>
                <li>• <strong>Auto-fill only applies to RFID-authenticated sessions.</strong> Walk-in residents fill forms manually.</li>
                <li>• <strong>Missing profile data leaves a blank</strong> in the output document — no error is thrown.</li>
              </ul>
            </div>
          </div>
        </template>
        ...
        <template v-if="activeSection === 'privacy'">
          ...
          <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <div class="bg-blue-600 px-5 py-4">
              ...
              <p class="text-blue-100 text-xs mt-0.5">Effective as of Deployment Date · Poblacion I, Amadeo, Cavite</p>
            </div>
            <div class="p-5 space-y-4 text-sm text-gray-600 leading-relaxed">
              <section>
                <h4 class="font-semibold text-gray-800 mb-1">1. Data Controller</h4>
                <p>The Barangay of Poblacion I, Amadeo, Cavite ("the Barangay") is the data controller for all personal information collected through the Barangay Transaction Kiosk and Admin Dashboard System. Staff designated by the Barangay Captain are authorized data processors.</p>
              </section>
              <section>
                <h4 class="font-semibold text-gray-800 mb-1">2. Information We Collect</h4>
                <p class="mb-2">The system collects the following from residents:</p>
                <ul class="space-y-1 ml-4 text-xs">
                  <li>• <strong>Identity:</strong> Full name, date of birth, gender, suffix</li>
                  <li>• <strong>Contact:</strong> Mobile phone number, email address</li>
                  <li>• <strong>Address:</strong> Complete residential address, purok/sitio</li>
                  <li>• <strong>Residency:</strong> Years of residency, residency start date</li>
                  <li>• <strong>Photo:</strong> ID photo captured at the kiosk camera</li>
                  <li>• <strong>System:</strong> RFID card UID, transaction history, submitted requests</li>
                </ul>
              </section>
              <section>
                <h4 class="font-semibold text-gray-800 mb-1">3. Legal Basis & Purpose</h4>
                <p>Data is collected under RA 7160 (Local Government Code) for: processing official barangay documents, verifying resident identity, sending SMS transaction notifications, maintaining resident records, and DILG reporting compliance.</p>
              </section>
              <section>
                <h4 class="font-semibold text-gray-800 mb-1">4. Data Storage & Security</h4>
                <p>All data is stored locally on the Raspberry Pi server within the barangay hall. <strong>No resident data is transmitted to any third-party cloud service.</strong> Security measures include RFID access control, PIN-based admin authentication, LAN isolation, and full audit logging.</p>
              </section>
              <section>
                <h4 class="font-semibold text-gray-800 mb-1">5. Data Sharing</h4>
                <p>Resident data shall not be sold or disclosed to third parties except when legally required by a competent authority, when shared within the LGU of Amadeo for official purposes, or in anonymized form for government statistical reporting.</p>
              </section>
              <section>
                <h4 class="font-semibold text-gray-800 mb-1">6. Resident Rights (RA 10173)</h4>
                <p>Residents may exercise their rights to: <strong>access</strong> their data, <strong>correct</strong> inaccurate information, <strong>request erasure</strong> subject to legal retention, and <strong>object</strong> to processing under certain conditions. Submit requests in writing to the Barangay Secretary.</p>
              </section>
              <section>
                <h4 class="font-semibold text-gray-800 mb-1">7. SMS Consent</h4>
                <p class="text-xs">By registering a mobile number, residents consent to receive official SMS notifications. Residents may opt out via written request to the Barangay Secretary.</p>
              </section>
              <section>
                <h4 class="font-semibold text-gray-800 mb-1">8. Data Retention</h4>
                <p class="text-xs">Data is retained per applicable Philippine law and LGU record-keeping policies. Transaction logs are kept for a minimum of five (5) years.</p>
              </section>
            </div>
          </div>
          ...
          <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <div class="bg-gray-700 px-5 py-4">
              ...
              <p class="text-gray-300 text-xs mt-0.5">For Barangay Admin Staff Users</p>
            </div>
            <div class="p-5 space-y-4 text-sm text-gray-600 leading-relaxed">
              <section>
                <h4 class="font-semibold text-gray-800 mb-1">1. Authorized Users</h4>
                <p class="text-xs">Access is restricted to staff designated by the Barangay Captain. Unauthorized use may violate RA 10175 (Cybercrime Prevention Act of 2012).</p>
              </section>
              <section>
                <h4 class="font-semibold text-gray-800 mb-1">2. Credential Responsibility</h4>
                <p class="text-xs">Admin accounts are personal and non-transferable. You are responsible for all actions performed under your credentials. Do not share passwords. Report suspected unauthorized access immediately.</p>
              </section>
              <section>
                <h4 class="font-semibold text-gray-800 mb-1">3. Acceptable Use</h4>
                <p class="mb-1.5 text-xs">The system must only be used for legitimate barangay service delivery. Prohibited activities include:</p>
                <ul class="space-y-1 ml-4 text-xs">
                  <li>• Accessing or modifying resident data without official need</li>
                  <li>• Creating false or fraudulent records or transaction entries</li>
                  <li>• Using the SMS system for personal or non-official messages</li>
                  <li>• Attempting to bypass security controls or access other LAN devices</li>
                  <li>• Exporting or copying resident data for personal or unauthorized use</li>
                </ul>
              </section>
              <section>
                <h4 class="font-semibold text-gray-800 mb-1">4. Audit & Monitoring</h4>
                <p class="text-xs">All actions in this dashboard are logged in the audit trail, including data modifications, document approvals, and logins. Logs are accessible to the Barangay Captain for compliance and accountability review.</p>
              </section>
              <section>
                <h4 class="font-semibold text-gray-800 mb-1">5. System Availability</h4>
                <p class="text-xs">The system operates on local infrastructure and does not guarantee 100% uptime. Power outages, hardware failures, or planned maintenance may cause temporary unavailability.</p>
              </section>
              <section>
                <h4 class="font-semibold text-gray-800 mb-1">6. Disclaimer of Warranty</h4>
                <p class="text-xs">This system is deployed as a thesis prototype. It is provided "as-is" without warranty of any kind. The development team shall not be liable for consequential damages arising from system use, data loss, or inaccuracies in generated documents.</p>
              </section>
              <section>
                <h4 class="font-semibold text-gray-800 mb-1">7. Amendments</h4>
                <p class="text-xs">These terms may be updated by barangay administration as the system evolves. Continued use constitutes acceptance of any revised terms.</p>
              </section>
              <div class="pt-4 border-t border-gray-100">
                <p class="text-xs text-gray-400 text-center leading-relaxed">
                  Developed in partial fulfillment of the requirements for the degree of Bachelor of Science in Computer Engineering<br/>
