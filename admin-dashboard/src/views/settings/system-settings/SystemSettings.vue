...
// ── Watchers (top-level — auto-cleaned up on unmount) ─────────────────────────
 
// FIX 1: Sync activeTab from URL query param on navigation.
// This watcher replaces the one that was incorrectly nested inside onMounted().
watch(
  () => route.query.tab,
  (newTab) => {
    if (newTab === "audit") {
...
// FIX 2: Load system logs lazily when the tab is first activated.
watch(activeTab, (val) => {
  if (val === "system-logs" && auth.isSuperAdmin && systemLogs.value.length === 0) {
    loadSystemLogs();
  }
});
  ...
  <div class="flex flex-col p-6 bg-white rounded-md w-full h-full overflow-hidden">
    ...
    <div class="flex justify-between items-center border-b border-gray-200">
      <n-tabs v-model:value="activeTab" type="line" animated class="flex-grow">
        ...
        <!-- Audit Log -->
        <n-tab-pane name="audit" tab="Audit Log" display-directive="show">
          <div class="overflow-y-auto h-[calc(100vh-260px)] pr-2 pt-6 flex flex-col w-full">
        ...
        <n-tab-pane v-if="auth.isSuperAdmin" name="system-logs" tab="System Logs" display-directive="show">
          <div class="overflow-y-auto h-[calc(100vh-260px)] pr-2 pt-6 flex flex-col w-full gap-5">
 
            <!-- Summary Cards -->
            <div class="grid grid-cols-4 gap-3">
              <div
                v-for="level in ['info','warning','error','critical']" :key="level"
            ...
            <div class="flex items-center gap-3 flex-wrap">
              <n-select
                ...
                placeholder="Source" clearable size="small" style="width:140px"
              ...
              <n-select
                ...
                placeholder="Level" clearable size="small" style="width:140px"
                @update:value="onSystemLogFilterChange"
              />
              <n-select
                ...
                placeholder="Category" clearable size="small" style="width:160px"
                @update:value="onSystemLogFilterChange"
              />
            ...
            <div v-else class="overflow-x-auto w-full">
              <table class="w-full text-left border-collapse">
                ...
                <tbody>
                  <tr
                    v-for="(log, index) in systemLogs" :key="index"
                    ...
                    <td class="py-4 px-4">
                      <span class="inline-flex items-center gap-1.5 text-[13px] text-gray-700 capitalize">
                        <span class="w-2 h-2 rounded-full flex-shrink-0"
                          :class="{
                            'bg-indigo-500': log.source === 'admin',
                            'bg-teal-500':   log.source === 'kiosk',
                            'bg-gray-400':   log.source === 'system',
                          }"
                        ></span>
                        {{ log.source }}
                      </span>
