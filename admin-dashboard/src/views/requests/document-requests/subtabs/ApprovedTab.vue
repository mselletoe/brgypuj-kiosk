<script setup>
/**
 * @file views/requests/document-requests/subtabs/ApprovedTab.vue
 * @description Manages document and RFID requests that have been approved.
 * Supports PDF viewing, SMS notifications for pickup, document release,
 * and undoing approval (moving back to pending).
...
const fetchApprovedRequests = async () => {
  ...
  try {
    ...
    const allRequests = response.data.map((req) => ({
      ...
      type:
        req.doctype_id === null || req.doctype_name.toUpperCase() === "RFID"
          ? "rfid"
          : "document",
      status: req.status.toLowerCase(),
      requestType: req.doctype_name,
      requester:
        req.doctype_id === null && req.form_data?.session_rfid === "Guest Mode"
      ...
      amount:
        req.payment_status !== "free" ? String(req.price ?? "0.00") : null,
  ...
  try {
    ...
    if (request) {
      await createAuditLog(
        "Released Document",
        `DOC-${request.transaction_no} - ${request.requestType}`,
        "doc",
      );
    }
...
const handleUndo = (id) => {
  openConfirmModal("Move this request back to pending?", async () => {
    try {
...
const filteredRequests = computed(() => {
  ...
  if (props.filters?.documentType) {
    result = result.filter(
      (req) => req.raw.doctype_id === props.filters.documentType,
    );
  }
...
<template>
  <div class="space-y-4 h-full">
    ...
    <div v-if="isLoading" class="flex flex-col items-center justify-center w-full h-full min-h-[300px] gap-4">
      ...
      <p class="text-gray-500 font-medium">Loading approved requests...</p>
    </div>
