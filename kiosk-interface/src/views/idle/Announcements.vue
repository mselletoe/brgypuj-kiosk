<script setup>
/**
 * @file views/idle/Announcements.vue
 * @description Kiosk idle/attract screen. Displays active barangay announcements
 * as a full-screen auto-advancing slideshow. Any tap on the screen navigates
 * to the login options screen. Polls for new announcements every 5 minutes
 * and auto-retries on fetch failure after 10 seconds.
 */
...
const prevSlide = () => {
  if (announcements.value.length === 0) return
  current.value = (current.value - 1 + announcements.value.length) % announcements.value.length
}
    ...
    <template v-else>
      ...
      <div class="relative z-10 h-full flex flex-col">
            ...
            <div v-if="announcements.length" :key="current" class="max-w-[60%]">
              ...
              <div class="mt-6 space-y-2">
                ...
                <p class="text-white text-[22px] opacity-95 leading-[1.3] flex items-center gap-3">
                  ...
                  <span>{{ formatDate(announcements[current]?.event_date) }}, {{ formatDay(announcements[current]?.event_date) }}</span>
                </p>
                <!-- Time -->
                <p v-if="announcements[current]?.event_time" class="text-white text-[22px] opacity-95 leading-[1.3] flex items-center gap-3">
                  <svg class="w-6 h-6 opacity-75 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
        ...
        <div class="flex items-center justify-between px-6 pb-10">
          <!-- Previous button -->
          <button v-if="announcements.length > 1" @click.stop.prevent="prevSlide" class="text-white transition-all duration-300 hover:scale-110 hover:opacity-100 opacity-70">
          ...
          <!-- Next button -->
          <button v-if="announcements.length > 1" @click.stop.prevent="nextSlide" class="text-white transition-all duration-300 hover:scale-110 hover:opacity-100 opacity-70">
            <svg class="w-16 h-16" fill="currentColor" viewBox="0 0 20 20">
