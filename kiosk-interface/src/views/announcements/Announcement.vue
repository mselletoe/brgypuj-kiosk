  ...
  <div class="announcement-page w-full min-h-screen px-10 py-6 flex flex-col">
    ...
    <header class="flex items-center justify-between mb-6">
      ...
      <div class="flex items-center gap-5">
        <div @click="toggleLang" class="w-36 h-12 bg-[#49759B] rounded-2xl flex cursor-pointer p-1" style="position: relative;">
          <div class="rounded-xl bg-white" style="position: absolute; top: 4px; bottom: 4px; width: calc(50% - 4px); transition: transform 0.3s cubic-bezier(0.25, 0.46, 0.45, 0.94);" :style="{ transform: currentLang === 'ENG' ? 'translateX(calc(100% + 0px))' : 'translateX(0px)' }"></div>
          <div class="flex-1 flex items-center justify-center font-bold rounded-xl" style="position: relative; z-index: 1; transition: color 0.3s ease;" :style="{ color: currentLang === 'FIL' ? '#49759B' : 'white' }">FIL</div>
          <div class="flex-1 flex items-center justify-center font-bold rounded-xl" style="position: relative; z-index: 1; transition: color 0.3s ease;" :style="{ color: currentLang === 'ENG' ? '#49759B' : 'white' }">ENG</div>
        </div>
      </div>
    </header>
    ...
    <main class="flex flex-col flex-1">
      ...
      <template v-else>
          ...
          <div class="flex-1 overflow-hidden" style="mask-image: linear-gradient(to right, transparent 0%, black 15%, black 85%, transparent 100%); -webkit-mask-image: linear-gradient(to right, transparent 0%, black 15%, black 85%, transparent 100%);">
            <div class="flex items-center justify-center">
              <div class="flex flex-shrink-0" :style="{ gap: `${GAP}px`, transform: `translateX(${translateX}px)`, transition: isAnimating ? 'transform 0.45s cubic-bezier(0.25, 0.46, 0.45, 0.94)' : 'none' }">
                <div v-for="item in windowItems" :key="`${item.id}-${item._offset}`" class="h-[300px] flex-shrink-0 text-white rounded-3xl overflow-hidden" :style="{ width: `${CARD_W}px`, backgroundImage: item.image_base64 ? `url(${getImageUrl(item.image_base64)})` : 'none', backgroundColor: item.image_base64 ? undefined : '#03335C', backgroundSize: 'cover', backgroundPosition: 'center', position: 'relative' }">
        ...
        <div class="flex justify-center gap-2 mt-6">
          <button v-for="(item, index) in announcements" :key="item.id" @click="setSlide(index)" class="h-3 rounded-full transition-all duration-300 ease-in-out" :class="index === displayIndex ? 'bg-[#1B5886] w-7' : 'border-[#1B5886] w-3 border-[2px]'" />
        </div>
      </template>
    </main>
  </div>
