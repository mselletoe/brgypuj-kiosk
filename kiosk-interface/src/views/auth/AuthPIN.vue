...
const triggerToast = (message, success = false) => {
  toastMessage.value = message; isSuccess.value = success; showToast.value = true
  setTimeout(() => { showToast.value = false }, 1500)
}

const isAdminMode     = computed(() => rfidRegStore.isAdminMode)
const title           = computed(() => { if (isAdminMode.value) return 'Admin Access Required'; return `Welcome, ${authStore.tempResident?.first_name || 'Resident'}` })
const subtitle        = computed(() => { if (isAdminMode.value) return 'Enter the admin passcode to register this new RFID card.'; if (!authStore.tempHasPin) return 'Please set your 4-digit PIN'; return 'Please enter your PIN to continue' })
const inputLabel      = computed(() => { if (isAdminMode.value) return 'Enter Admin Passcode'; if (!authStore.tempHasPin) return 'Enter New 4-digit PIN'; return 'Enter 4-digit PIN' })
const buttonLabel     = computed(() => isAdminMode.value ? 'Confirm' : 'Authenticate')
const pinDisplay      = computed(() => { if (!pin.value.length) return ''; return showPin.value ? pin.value : '• '.repeat(pin.value.length).trim() })
const confirmPinDisplay = computed(() => { if (!confirmPin.value.length) return ''; return showPin.value ? confirmPin.value : '• '.repeat(confirmPin.value.length).trim() })
const showConfirmField = computed(() => !isAdminMode.value && !authStore.tempHasPin)
const isPinComplete   = computed(() => { if (isAdminMode.value) return pin.value.length === PIN_LENGTH; if (!authStore.tempHasPin) return pin.value.length === PIN_LENGTH && confirmPin.value.length === PIN_LENGTH; return pin.value.length === PIN_LENGTH })

const onKeypress  = (key) => { const useConfirm = showConfirmField.value && pin.value.length === PIN_LENGTH; const target = useConfirm ? confirmPin : pin; if (target.value.length < PIN_LENGTH) target.value += key }
const onClear     = () => { pin.value = ''; confirmPin.value = '' }
const onBackspace = () => { if (showConfirmField.value && confirmPin.value.length > 0) { confirmPin.value = confirmPin.value.slice(0, -1) } else if (pin.value.length > 0) { pin.value = pin.value.slice(0, -1) } }
...
const submitPin = async () => {
  ...
  try {
    ...
    const { valid } = await verifyPin({ resident_id: authStore.tempResident.id, pin: pin.value })
    if (!valid) { triggerToast('Invalid PIN'); onClear(); return }
    authStore.confirmRFIDLogin()
    triggerToast('Login successful!', true)
    router.replace('/home')
  } catch (err) { triggerToast('An error occurred. Please try again.'); onClear() }
}

const goBack = () => { if (isAdminMode.value) rfidRegStore.clearAll(); router.push('/login-rfid') }
      ...
      <div class="flex flex-1 flex-col items-center justify-center pl-12 border-l border-gray-200 relative">
        <div class="w-full max-w-xs">
          ...
          <div class="relative w-full">
            <input :value="pinDisplay" type="text" readonly class="w-full h-14 bg-gray-50 border border-gray-300 rounded-lg text-3xl text-center font-light focus:outline-none" :class="showPin ? 'tracking-normal' : 'tracking-widest'" />
        ...
        <div class="w-full max-w-xs">
          ...
          <button @click="submitPin" :disabled="!isPinComplete" :class="['w-full h-14 text-xl font-semibold rounded-lg mt-4 transition-colors text-white', isPinComplete ? 'bg-[#013C6D] hover:bg-[#012f5a]' : 'bg-gray-400 cursor-not-allowed']">
            {{ buttonLabel }}
          </button>
        </div>
