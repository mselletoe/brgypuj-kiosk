...
async def approve_document_request(request_id: int, db: Session = Depends(get_db)):
    success = approve_request(db, request_id)
    if not success:
        raise HTTPException(status_code=404, detail="Request not found")
    await sse_manager.broadcast("request_updated", {"id": request_id, "status": "approved"})
    return {"detail": "Request approved"}


@router.post("/requests/{request_id}/reject")
async def reject_document_request(request_id: int, db: Session = Depends(get_db)):
    success = reject_request(db, request_id)
    if not success:
        raise HTTPException(status_code=404, detail="Request not found")
    await sse_manager.broadcast("request_updated", {"id": request_id, "status": "rejected"})
    return {"detail": "Request rejected"}


@router.post("/requests/{request_id}/release")
async def release_document_request(request_id: int, db: Session = Depends(get_db)):
    success = release_request(db, request_id)
    if not success:
        raise HTTPException(status_code=404, detail="Request not found")
    await sse_manager.broadcast("request_updated", {"id": request_id, "status": "released"})
    return {"detail": "Request released"}


@router.post("/requests/{request_id}/mark-paid")
async def mark_request_as_paid(request_id: int, db: Session = Depends(get_db)):
    success = mark_request_paid(db, request_id)
    if not success:
        raise HTTPException(status_code=404, detail="Request not found")
    await sse_manager.broadcast("request_payment_updated", {"id": request_id, "payment_status": "paid"})
    return {"detail": "Marked as paid"}


@router.post("/requests/{request_id}/mark-unpaid")
async def mark_request_as_unpaid(request_id: int, db: Session = Depends(get_db)):
    success = mark_request_unpaid(db, request_id)
    if not success:
        raise HTTPException(status_code=404, detail="Request not found")
    await sse_manager.broadcast("request_payment_updated", {"id": request_id, "payment_status": "unpaid"})
    return {"detail": "Marked as unpaid"}


@router.post("/requests/{request_id}/undo")
async def undo_document_request(request_id: int, db: Session = Depends(get_db)):
    try:
        success = undo_request(db, request_id)
        if not success:
            raise HTTPException(status_code=404, detail="Request not found")
        await sse_manager.broadcast("request_updated", {"id": request_id, "status": "pending"})
...
async def bulk_undo_document_requests(ids: list[int] = Body(...), db: Session = Depends(get_db)):
    updated_count = bulk_undo_requests(db, ids)
    await sse_manager.broadcast("requests_bulk_undone", {"ids": ids})
    return {"detail": f"{updated_count} requests reverted"}


@router.delete("/requests/{request_id}")
async def delete_document_request(request_id: int, db: Session = Depends(get_db)):
    success = delete_request(db, request_id)
    if not success:
        raise HTTPException(status_code=404, detail="Request not found")
    await sse_manager.broadcast("request_deleted", {"id": request_id})
    return {"detail": "Request deleted"}


@router.post("/requests/bulk-delete")
async def bulk_delete_document_requests(ids: list[int] = Body(...), db: Session = Depends(get_db)):
    deleted_count = bulk_delete_requests(db, ids)
    await sse_manager.broadcast("requests_bulk_deleted", {"ids": ids})
